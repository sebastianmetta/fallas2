...
public class EncadenamientoHaciaAtras2 {
	...
	public EncadenamientoHaciaAtras2(BaseDeConocimiento baseDeConocimiento, List<Premisa> hechosIniciales) {
	...
	/*
	public List<Premisa> evaluar2(int nroIteracion, List<Premisa> hechosIniciales, Premisa hipotesisInicial) {
		int nroIteracionActual = nroIteracion;
		
		// Conocimientos adquiridos
		if (conocimientoAdquirido == null) {
			conocimientoAdquirido = new ArrayList<Premisa>();
			conocimientoAdquirido.addAll(hechosIniciales);
		}

		// Hipotesis a evaluar
		if (pilaHipotesisProcesar == null) {
			pilaHipotesisProcesar = new LinkedList<Premisa>();
		}
		pilaHipotesisProcesar.addLast(hipotesisInicial);

		// Hipotesis a evaluadas
		if (hipotesisProcesadas == null) {
			hipotesisProcesadas = new ArrayList<Premisa>();
		}
		...
		if (conocimientoAdquirido.contains(hipotesisActual)) {
			...
			while (continuar) {
				...
				if (!reglasAplicables.isEmpty()) {
					...
					// verificar si las premisas de la regla elegida se
					// encuentran
					...
					if (!premisasEnBaseConocimiento) {
						...
						// regla y se actualiza el banco de conocimiento
						List<Premisa> nuevasHipotesisAVerificar = reglaADisparar.obtenerPremisasNoCoincidentes(conocimientoAdquirido);
						//pilaHipotesisProcesar.addAll(nuevasHipotesisAVerificar);
						...
						for (Premisa premisa : nuevasHipotesisAVerificar) {
							nroIteracion++;
							evaluar(nroIteracion, conocimientoAdquirido, premisa);
					...
					if (premisasEnBaseConocimiento) {
						...
						log.info("La hipotesis " + hipotesisActual.toString() + " NO es deducible mediante la regla " + reglaADisparar.toString());
						log.info("Se buscara una nueva regla que permita deducir la hipotesis " + hipotesisActual);
					}
